...
/** Public route prefixes that are accessible without login */
const publicPrefixes = ['/landing', '/campaigns', '/berita', '/tentang', '/kontak']
...
export default auth((req) => {
    ...
    /** Allow root page (redirects to landing) */
    ...
    /** Allow public routes and routes with public prefixes */
    if (isPublicRoute || isPublicPrefix) return

    /** Redirect to sign-in if not signed in and not public route */
    if (!isSignedIn) {
